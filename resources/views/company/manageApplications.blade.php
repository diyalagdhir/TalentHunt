                               ...
                               <tr>
                                    ...
                                    <td class="text-center">
                                        @if ($job->application_status == "Viewed")
                                            ...
                                            data-bs-target="#scheduleInterviewModal" data-id="{{ $job->app_id }}">Interview </button>
                                        @else
      ...
      <div class="modal-content">
          ...
          <div class="modal-body">
              <form id="scheduleMeetingForm" method="POST" action="{{ route('schedule.interview') }}">
                  @csrf
                  <input type="hidden" id="interview_app_id" name="interview_app_id" value="">
...
<script>
  $(document).ready(function () {
        $("#scheduleInterviewModal").on("show.bs.modal", function (event) {
            ...
            var appId = button.data("id"); // Extract app_id from data attribute
            //console.log(appId);
            console.log("App ID before modal open:", appId);
            $("#interview_app_id").val(appId); // Set app_id in hidden input field

            console.log("App ID in hidden input:", $("#app_id").val());
        });

        $("#scheduleMeetingForm").submit(function (event) {
            var appIdFinal = $("#interview_app_id").val();
